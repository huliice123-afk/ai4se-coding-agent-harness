    ...
    private Map<String, Object> buildRequestBody(List<Message> messages, List<Tool> tools) {
        ...
        if (!tools.isEmpty()) {
            ...
            for (Tool tool : tools) {
                ...
                td.put("name", tool.getName());
                td.put("description", tool.getDescription());
                try {
                    td.put("input_schema", mapper.readValue(tool.getParameters(), Map.class));
                } catch (IOException e) {
    ...
    private LlmResponse parseResponse(String responseBody) throws IOException {
        ...
        for (Map<String, Object> block : content) {
            ...
            if ("tool_use".equals(type)) {
                ...
                String args = input != null ? mapper.writeValueAsString(input) : "{}";
                return new LlmResponse(null, name, args, stopReason);
            }
            if ("text".equals(type)) {
